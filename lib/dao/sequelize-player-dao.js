...
SequelizePlayerDAO.prototype.setWinner = function (uuid, callback) {
    _playerModel.find(uuid).complete(function (err, res) {
        if (!err) {
            res.updateAttributes({previousWinner: true}).complete(function (e) {
                if (callback) {
                    callback(res, e);
                }
            })
...
SequelizePlayerDAO.prototype.removeWinner = function (uuid, callback) {
    _playerModel.find(uuid).complete(function (err, res) {
        if (!err) {
            res.updateAttributes({previousWinner: false}).complete(function (e) {
                if (callback) {
                    callback(res, e);
                }
            })
...
SequelizePlayerDAO.prototype.leaderboard = function (callback) {
    _sequelize.query(
            "SELECT playerId, alias, previousWinner, COUNT(*) as collected, MAX(createdAt) as lastCode,MIN(createdAt) as firstCode, strftime('%s',MAX(createdAt)) - strftime('%s',MIN(createdAt)) as duration " +
            "FROM " +
            "    (SELECT playerId, previousWinner, alias, codeId, Scans.createdAt, title, gameIndex " + "" +
