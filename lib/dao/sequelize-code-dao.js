...
SequelizeCodeDAO.prototype.findById = function (id, callback) {
    _codeModel.find({where: ["id = ?", id]}).complete(function (err, code) {
        if (callback) {
            callback(code, err);
        }
    });
...
SequelizeCodeDAO.prototype.create = function (code, callback) {
    if (code.id) {
        ...
        _codeModel.create(code).complete(function (err, result) {
            if (callback) {
                callback(result, err);
            }
        });
...
SequelizeCodeDAO.prototype.remove = function (id, callback) {
    _codeModel.destroy({id : id}).complete(function (err) {
        if (callback) {
            callback(err);
        }
    });
